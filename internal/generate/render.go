...
func renderGenerated(modulePath string, resources []spec.Resource, opts ProjectOptions) (map[string][]byte, error) {
	...
	if opts.HasSession() {
		output, err := executeGoTemplate("internal/generated/permissions_gen.go", permissionsTemplate, all)
		if err != nil {
			return nil, fmt.Errorf("render internal/generated/permissions_gen.go: %w", err)
		}
		files["internal/generated/permissions_gen.go"] = output
		frontend, err := renderFrontendResources(resources)
		if err != nil {
			return nil, err
		}
		for name, output := range frontend {
			files[name] = output
		}
	}
...
func gormTag(opts ProjectOptions, field spec.Field) string {
	...
	if len(field.Enum) > 0 {
		...
		parts = append(parts, "check:"+field.Column+" IN ("+strings.Join(values, ", ")+")")
	}
...
const permissionsTemplate = `// Code generated by gobackend; DO NOT EDIT.

package generated

import "{{.Module}}/internal/platform/rbac"

var RolePermissions = rbac.RolePermissions{
	"admin": {
{{range .Resources}}		"{{.Package}}.read": {},
		"{{.Package}}.create": {},
		"{{.Package}}.update": {},
		"{{.Package}}.delete": {},
{{end}}	},
...
{{range .Resources}}	"GET /api/v1{{.Route}}": "{{.Package}}.read",
	"GET /api/v1{{.Route}}/:id": "{{.Package}}.read",
	"POST /api/v1{{.Route}}": "{{.Package}}.create",
	"PATCH /api/v1{{.Route}}/:id": "{{.Package}}.update",
	"DELETE /api/v1{{.Route}}/:id": "{{.Package}}.delete",
{{end}}}
`
