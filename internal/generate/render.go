...
func renderGenerated(modulePath string, resources []spec.Resource) (map[string][]byte, error) {
	...
	resourceTemplates := []struct {
		...
		{"repository_gen.go", repositoryTemplate},
		{"service_gen.go", serviceTemplate},
		{"handler_gen.go", handlerTemplate},
		{"routes_gen.go", routesTemplate},
...
const repositoryTemplate = `// Code generated by gobackend; DO NOT EDIT.
...
import (
	...
	"{{.Module}}/internal/platform/repository"
...
type Repository interface {
	List(context.Context, Filters) ([]{{.Resource.Name}}, int64, error)
	Get(context.Context, int64) ({{.Resource.Name}}, error)
	Create(context.Context, map[string]any) ({{.Resource.Name}}, error)
	Update(context.Context, int64, map[string]any) error
	Delete(context.Context, int64) error
}

type gormRepository struct {
	db *gorm.DB
	base *repository.Base[{{.Resource.Name}}]
}

func NewRepository(db *gorm.DB) Repository {
	return &gormRepository{db: db, base: repository.New[{{.Resource.Name}}](db)}
}

func (r *gormRepository) List(ctx context.Context, filters Filters) ([]{{.Resource.Name}}, int64, error) {
	db := r.db.WithContext(ctx).Model(&{{.Resource.Name}}{})
...
{{end}}{{range filterFields .Resource}}	if value, ok := filters.Exact[{{quote .Name}}]; ok { db = db.Where(repository.Equal(gormgen.{{$.Resource.Name}}.{{.GoName}}, value)) }
{{end}}	var total int64
...
func (r *gormRepository) Get(ctx context.Context, id int64) ({{.Resource.Name}}, error) { return r.base.Get(ctx, id) }
func (r *gormRepository) Create(ctx context.Context, values map[string]any) ({{.Resource.Name}}, error) {
	now := time.Now().UTC()
...
{{end}}	placeholders := make([]string, len(columns))
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var id int64
		if err := tx.Raw(query, arguments...).Scan(&id).Error; err != nil { return repository.Translate(tx, err) }
		item, err := repository.New[{{.Resource.Name}}](tx).Get(ctx, id)
...
func (r *gormRepository) Update(ctx context.Context, id int64, values map[string]any) error { return r.base.Update(ctx, id, values) }
func (r *gormRepository) Delete(ctx context.Context, id int64) error { return r.base.Delete(ctx, id) }
`

const serviceTemplate = `// Code generated by gobackend; DO NOT EDIT.
...
type Service interface {
	List(context.Context, Filters) ([]{{.Resource.Name}}, int64, error)
	Get(context.Context, int64) ({{.Resource.Name}}, error)
	Create(context.Context, Create{{.Resource.Name}}Input) ({{.Resource.Name}}, error)
	Update(context.Context, int64, Update{{.Resource.Name}}Input) ({{.Resource.Name}}, error)
	Delete(context.Context, int64) error
}
...
func (s *service) List(ctx context.Context, filters Filters) ([]{{.Resource.Name}}, int64, error) {
	items, total, err := s.repository.List(ctx, filters)
	if err != nil { return nil, 0, apperror.FromDatabase(err) }
	return items, total, nil
}

func (s *service) Get(ctx context.Context, id int64) ({{.Resource.Name}}, error) {
	item, err := s.repository.Get(ctx, id)
	if err != nil { return {{.Resource.Name}}{}, apperror.FromDatabase(err) }
	return item, nil
}

func (s *service) Create(ctx context.Context, input Create{{.Resource.Name}}Input) ({{.Resource.Name}}, error) {
	if details := validateCreate(input); len(details) > 0 { return {{.Resource.Name}}{}, apperror.Validation(details) }
...
{{end}}	created, err := s.repository.Create(ctx, values)
	if err != nil { return {{.Resource.Name}}{}, apperror.FromDatabase(err) }
	return created, nil
}

func (s *service) Update(ctx context.Context, id int64, input Update{{.Resource.Name}}Input) ({{.Resource.Name}}, error) {
	if details := validateUpdate(input); len(details) > 0 { return {{.Resource.Name}}{}, apperror.Validation(details) }
	values := map[string]any{}
{{range .Resource.Fields}}	if input.{{.GoName}}.IsSet() { if input.{{.GoName}}.IsNull() { values[{{quote .Column}}] = nil } else if value, ok := input.{{.GoName}}.Value(); ok { {{if eq .Type "time"}}value = value.UTC(); {{end}}values[{{quote .Column}}] = value } }
{{end}}	if len(values) == 0 { return {{.Resource.Name}}{}, apperror.BadRequest("empty_update", "at least one field must be provided") }
	if err := s.repository.Update(ctx, id, values); err != nil { return {{.Resource.Name}}{}, apperror.FromDatabase(err) }
	return s.Get(ctx, id)
}

func (s *service) Delete(ctx context.Context, id int64) error {
	return apperror.FromDatabase(s.repository.Delete(ctx, id))
}
`

const handlerTemplate = `// Code generated by gobackend; DO NOT EDIT.
...
{{end}}	items, total, serviceErr := h.service.List(c.Request().Context(), Filters{Page: page, PageSize: pageSize, Sort: sortValue, Query: c.QueryParam("q"), Exact: exact})
	if serviceErr != nil { return httpx.WriteError(c, serviceErr) }
...
func (h *Handler) Get(c *echo.Context) error {
	...
	item, serviceErr := h.service.Get(c.Request().Context(), id)
	if serviceErr != nil { return httpx.WriteError(c, serviceErr) }
...
func (h *Handler) Create(c *echo.Context) error {
	...
	item, serviceErr := h.service.Create(c.Request().Context(), input)
	if serviceErr != nil { return httpx.WriteError(c, serviceErr) }
...
func (h *Handler) Update(c *echo.Context) error {
	...
	if err := httpx.DecodeJSON(c.Request(), &input); err != nil { return httpx.WriteError(c, apperror.BadRequest("invalid_json", "request body must be valid JSON")) }
	item, serviceErr := h.service.Update(c.Request().Context(), id, input)
	if serviceErr != nil { return httpx.WriteError(c, serviceErr) }
	return httpx.WriteData(c, http.StatusOK, item)
}
...
func (h *Handler) Delete(c *echo.Context) error {
	id, err := parseID(c.Param("id")); if err != nil { return httpx.WriteError(c, err) }
	if serviceErr := h.service.Delete(c.Request().Context(), id); serviceErr != nil { return httpx.WriteError(c, serviceErr) }
...
const routesTemplate = `// Code generated by gobackend; DO NOT EDIT.
