...
if [ -z "${DATABASE_URL:-}" ]; then
  container_name="go-backend-kit-session-e2e-$$"
  created_container=$(docker run -d --name "$container_name" \
    -e POSTGRES_DB=kit_session_test \
    -e POSTGRES_USER=kit \
...
project="$tmp/session-api"
go build -o "$binary" ./cmd/gobackend
GOBACKEND_DEVELOPMENT_REPLACE="$root" "$binary" new "$project" --module example.com/session-api --database postgres --auth session
...
./scripts/atlas.sh migrate diff session_auth --env local
...
TEST_RATE_DATABASE_URL="$DATABASE_URL" TEST_RATE_DISPOSABLE=1 go test -race ./internal/platform/auth -run '^TestPostgresSharedLimiter$' -count=1
